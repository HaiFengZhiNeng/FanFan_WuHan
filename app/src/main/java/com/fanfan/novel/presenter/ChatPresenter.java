...
public class ChatPresenter extends IChatPresenter implements Observer {
    ...
    private TIMConversation conversationServer2;
    private TIMConversation conversationServer3;
    private TIMConversation conversationServer4;
    ...
    public void start() {
        ...
        conversationServer2 = TIMManager.getInstance().getConversation(TIMConversationType.C2C, robotInfo.getServer2Id());
        conversationServer3 = TIMManager.getInstance().getConversation(TIMConversationType.C2C, robotInfo.getServer3Id());
        conversationServer4 = TIMManager.getInstance().getConversation(TIMConversationType.C2C, robotInfo.getServer4Id());
        //注册消息监听
...
//        MessageEvent.getInstance().onNewMessage(message);
    ...
    public void sendServerMessage(String question) {
        ...
        conversationServer2.sendMessage(msg, new TIMValueCallBack<TIMMessage>() {
            @Override
            public void onError(int code, String desc) {//发送消息失败
                //错误码code和错误描述desc，可用于定位请求失败原因
                //错误码code含义请参见错误码表
        ...
        conversationServer3.sendMessage(msg, new TIMValueCallBack<TIMMessage>() {
            @Override
            public void onError(int code, String desc) {//发送消息失败
                //错误码code和错误描述desc，可用于定位请求失败原因
                //错误码code含义请参见错误码表
        ...
        conversationServer4.sendMessage(msg, new TIMValueCallBack<TIMMessage>() {
            @Override
            public void onError(int code, String desc) {//发送消息失败
                //错误码code和错误描述desc，可用于定位请求失败原因
                //错误码code含义请参见错误码表
...
//                    Print.e(conversation.getPeer());
                ...
                }  else if (msg == null || msg.getConversation().getPeer().equals(conversationServer2.getPeer())
                        && msg.getConversation().getType() == conversationServer2.getType()) {
                    if (msg == null || msg.getConversation().getPeer().equals(conversationServer2.getPeer())
                            && msg.getConversation().getType() == conversationServer2.getType()) {
                        if (msg == null) {
                            return;
                        }
                ...
                } else if (msg == null || msg.getConversation().getPeer().equals(conversationServer3.getPeer())
                        && msg.getConversation().getType() == conversationServer3.getType()) {
                    if (msg == null || msg.getConversation().getPeer().equals(conversationServer3.getPeer())
                            && msg.getConversation().getType() == conversationServer3.getType()) {
                        if (msg == null) {
                            return;
                        }
                ...
                } else if (msg == null || msg.getConversation().getPeer().equals(conversationServer4.getPeer())
                        && msg.getConversation().getType() == conversationServer4.getType()) {
                    if (msg == null || msg.getConversation().getPeer().equals(conversationServer4.getPeer())
                            && msg.getConversation().getType() == conversationServer4.getType()) {
                        if (msg == null) {
                            return;
                        }
