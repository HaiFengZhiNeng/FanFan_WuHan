...
import com.fanfan.novel.ui.camera.state.CameraMachine;
...
public class CameraView extends FrameLayout implements
        com.fanfan.novel.ui.camera.view.CameraView,
        CameraInterface.CameraOpenOverCallback,
        SurfaceHolder.Callback,
        View.OnClickListener,
        CaptureLayout.TypeListener,
        CaptureButton.CaptureListener {
    ...
    private CaptureLayout.ClickListener leftClickListener;
    private CaptureLayout.ClickListener rightClickListener;
    ...
    public CameraView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        duration = a.getResourceId(R.styleable.CameraView_duration_max, 10 * 1000);
        a.recycle();
        ...
        mCaptureLayout.setLeftClickListener(new CaptureLayout.ClickListener() {
            @Override
            public void onClick() {
        ...
        mCaptureLayout.setRightClickListener(new CaptureLayout.ClickListener() {
            @Override
            public void onClick() {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (event.getPointerCount() == 2) {
                    ...
                    //Math.pow(x,y) 返回底数的指定次幂     返回以x的y次幂，等同于xy的数值表达式
                    //Math.sqrt  返回x的平方根            返回x的平方根
    ...
    private void updateVideoViewSize(float videoWidth, float videoHeight) {
        if (videoWidth > videoHeight) {
            LayoutParams videoViewParam = new LayoutParams(LayoutParams.MATCH_PARENT, (int) ((videoHeight / videoWidth) * getWidth()));
        ...
        switch (type) {
            case TYPE_VIDEO:
                ...
                if (mCameraLisenter != null) {
                    mCameraLisenter.recordSuccess(videoUrl, mFirstFrame);
                }
                break;
            case TYPE_PICTURE:
                mPhoto.setVisibility(INVISIBLE);
    ...
    public void recordError() {
        if (mErrorListener != null) {
            mErrorListener.AudioPermissionError();
        }
    }
    ...
    public void setErrorLisenter(ErrorListener errorLisenter) {
        mErrorListener = errorLisenter;
        CameraInterface.getInstance().setErrorLinsenter(errorLisenter);
    }

    public void setLeftClickListener(CaptureLayout.ClickListener clickListener) {
        leftClickListener = clickListener;
    }

    public void setRightClickListener(CaptureLayout.ClickListener clickListener) {
    ...
    public void setCameraLisenter(CameraListener jCameraLisenter) {
        mCameraLisenter = jCameraLisenter;
    }
